...
const { AccountId, Client, AccountInfoQuery, PrivateKey, TokenAssociateTransaction, TokenId } = require("@hashgraph/sdk");
...
class Utils {
  ...
  static async associateWithSigner(privateKey, tokenAddress, operatorId, operatorKey) {
    const genesisClient = await this.createLocalSDKClient(operatorId, operatorKey);
